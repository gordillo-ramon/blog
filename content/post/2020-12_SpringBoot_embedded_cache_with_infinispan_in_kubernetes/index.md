...
All of them are pretty simple to add to your project, and provides automatically useful features to make your microservice as simple as possible.

Although I am copying some code snippets here to explain the process, if you want to give it a try by yourself I recommend to clone [the repo]().

## Developing the microservice

## Maven file
For this example, I will use maven, but same can be implemented with graddle.
These are the artifacts thas should be added in [pom.xml]() for start developing the code:
...
### POJO
First, we should define the java object to serialize. I will use lombok annotations to create the setters/getters, and spring-docs for giving some context to the OpenAPI autogenerated documentation [Customer.java]().
...
```
It selects the spring-cache qualifier to be implemented. You can use also JCache with slightly differences in the annotations, at the moment of writing this article, you can see them on [this link](https://docs.spring.io/spring-framework/docs/current/reference/html/integration.html#cache-jsr-107) of the official spring documentation.
...
First step for an usual developer is test it in his own laptop (It works on my machine ¯\_(ツ)_/¯). So I have created an application.properties with simple configuration, which does not use nothing from kubernetes.
...
Good, everything works fine. The final test is using the OpenAPI UI or sending directly requests to the endpoints, to create some values and retrieve them. I recommend the UI as it is easier for a first try, although for unit testing it is prefered to call directly the endpoints.
...
That is, now it is using the folder where the configMap is mounted to select a new cache configuration specific for kubernetes and multireplica. The `cache-kubernetes.xml` is as follows:
...
- The marshaller between replicas is base on the JavaSerialization and the allow-list has been configured for allowing all classes. The recommended marshaller is protobuf, but I wanted to keep the code as simple as posible. For more information on marshallers, see [this document](https://infinispan.org/docs/stable/titles/developing/developing.html#marshalling).
...
As jkube will take the KUBECONFIG context from your shell, remember to set up it before running the maven goal.

If you are going to develop something similar, remember you may create maven profiles with goals that provides the diferent steps in an orderly way, so it will be more error prone in your CI/CD.